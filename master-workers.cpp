...
class Master {
    ...
    void operator()()
    {
        ...
        auto dag = generate_dag(split(pws_commands, '\n');
        int i = 0;
        for (auto const &deps : dag) {
            std::cout << "Line " << i << " is serving ";
            for (auto const &dep : deps.dependents) {
                std::cout << dep << " ";
            }
            std::cout << " and depends on " << deps.dependency_count << " lines..";
            std::cout << std::endl;
            ++i;
        }
